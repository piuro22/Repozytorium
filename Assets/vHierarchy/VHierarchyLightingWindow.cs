...
namespace VHierarchy
{
    public class VHierarchyLightingWindow : CustomPopupWindow
    {
        void OnGUI()
        {
            ...
            {
                var light = FindObjectsOfType<Light>().Where(r => r.type == LightType.Directional && r.gameObject.scene == EditorSceneManager.GetActiveScene()).FirstOrDefault();
            ...
            void ambientLight()
            {
                ...
                foreach (var r in Object.FindObjectsOfType<RenderSettings>())
                    r.RecordUndo();
