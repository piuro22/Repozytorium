...
namespace E2C.ChartBuilder
{
    public class E2ChartDataInfoDateTime : E2ChartDataInfoLinear
    {
        ...
        public void ComputeRange(bool fullRange, string rMin, string rMax, string format)
        {
            ...
            if (fullRange)
            {
                ...
                if (minTick == long.MaxValue) { minTick = 0; maxTick = 1; }
                if (minTick >= maxTick) maxTick = minTick + 1;
                ...
                if (Mathf.Approximately(minPos, float.MaxValue)) 
                { 
                    minPos = DATETIME_VALUE_MIN; 
                    maxPos = DATETIME_VALUE_MAX; 
                }
            }
            else
            {
                System.DateTime dtMin = System.DateTime.ParseExact(rMin, format, cultureInfo);
                System.DateTime dtMax = System.DateTime.ParseExact(rMax, format, cultureInfo);
                minTick = dtMin.Ticks;
                maxTick = dtMax.Ticks;
                if (minTick >= maxTick) maxTick = minTick + 1;
